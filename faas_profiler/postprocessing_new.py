...
class GraphCache:
    ...
    def cache_graph(
        ...
        # if trace_id in self._graphes_by_id:
        #     breakpoint()
    ...
    def state(self):
        ...
        for tid, graph in self._graphes_by_id.items():
            print(f"{tid}: GRAPH ID {graph.graph['trace_id']}, PARENT: {graph.graph.get('parent_trace_id')} {str(graph)}, MEM {hex(id(graph))}")
...
def process_records() -> None:
    ...
    print(f"Found {len(config.storage.unprocessed_record_keys)} unprocessed records \n")
    ...
    assert len(config.storage.unprocessed_record_keys) == len(processed_records)
...
def process_record(
    ...
    print(f"-- Before {func_ctx.function_key} Trace ID {trace_ctx.trace_id} --")
    print(graph_cache.state())
    print("")
    ...
    graph.add_node(str(record.record_id), **{
        "type": FUNCTION_NODE,
        "label": f"{func_ctx.function_key} ({str(record.record_id)[:8]})",
    ...
    if trace_ctx.parent_id is not None:
        graph.add_edge(str(trace_ctx.parent_id), str(record.record_id), **{
            "type": "sync",
            "latency": 1,
            "label": "1 ms"
        })
    ...
    if in_ctx and in_ctx.resolvable:
        graph = resolve_inbound_context(record, graph, graph_cache, request_cache)


    if record.outbound_contexts:
        resolve_outbound_contexts(record, graph, graph_cache, request_cache)

    print(f"-- After {func_ctx.function_key} Trace ID {trace_ctx.trace_id} --")
    print(graph_cache.state())
    print("")
    # for g in graph_cache.get_all_graphes():
    #     pos = nx.planar_layout(g)
...
def resolve_inbound_context(
    ...
    if outbound_request:
        ...
        if parent_graph:
            ...
            if parent_graph.has_edge(str(parent_trace_ctx.record_id), str(record.record_id)):
                parent_graph.remove_edge(str(parent_trace_ctx.record_id), str(record.record_id))
            
            if trace_graph.has_edge(str(parent_trace_ctx.record_id), str(record.record_id)):
                trace_graph.remove_edge(str(parent_trace_ctx.record_id), str(record.record_id))
            ...
            merged_graph.add_node(mid_node_id, **{
                "type": SERVICE_NODE,
                "total_execution_time": parent_out_ctx.overhead_time + record.inbound_context.trigger_overhead_time
                # "label": f"{parent_out_ctx} \n {record.inbound_context}"
            })
            merged_graph.add_edge(str(parent_trace_ctx.record_id), mid_node_id, **{
            ...
            graph_cache.cache_graph(parent_graph.graph["trace_id"], merged_graph)
    ...
    for out_ctx in record.outbound_contexts:
        ...
        if inbound_request:
            ...
            if child_graph:
                ...
                if child_graph.has_edge(str(record.record_id), str(child_context.record_id)):
                    child_graph.remove_edge(str(record.record_id), str(child_context.record_id))

                if trace_graph.has_edge(str(record.record_id), str(child_context.record_id)):
                    trace_graph.remove_edge(str(record.record_id), str(child_context.record_id))
                ...
                merged_graph.add_edge(mid_node_id, str(child_context.record_id), **{
                ...
                graph_cache.cache_graph(trace_graph.graph["trace_id"], merged_graph)
...
def set_normalized_edge_weights(graph):
    ...
    weights = {
        k: max(min_size, max_size * v * (1.0 / max_latency)) for k, v in latencies.items()}
...
def set_normalized_node_weights(graph):
    ...
    weights = {
        k: max(min_size, max_size * v * (1.0 / max_time)) for k, v in execution_times.items()}
...
def normalized_graph_weights(graph):
    ...
    edge_weights = {
        k: max(min_edge, max_edge * v * (1.0 / max_time)) for k, v in latencies.items()}
